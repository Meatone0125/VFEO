...
from learning.modules import DAEGC
...
def main():
    ...
    parser.add_argument('--daeodir', default='results', help='Directory to store pretrained results')
    ...
    args.daemodel_path = args.daeodir + ''
    ...
    def train():
        ...
        # iterate over dataset in batches
            for bidx, (targets, GIs, clouds_data,edge_index) in enumerate(loader):
                ...
                adj = torch.sparse_coo_tensor(edge_index, torch.ones(edge_index.shape[1]),torch.Size([embeddings.shape[0], embeddings.shape[0]])).to_dense()  # [3840,3840]
                adj += torch.eye(adj.shape[0])
                adj = adj / adj.sum(dim=1, keepdim=True)
                M = get_M(adj).cuda()
                adj = adj.clone().detach().cuda()
                with torch.no_grad():
                    _, z = dae.gat(embeddings, adj, M)
                kmeans = KMeans(n_clusters=new_num_clusters, n_init=20)
                kmeans.fit_predict(z.data.cpu().numpy())
                dae.cluster_layer.data = torch.tensor(kmeans.cluster_centers_).cuda()
                A_pred, z, q = dae(embeddings, adj, M)
                outputs = model.ecc(z)
    ...
    def eval(is_valid = False):
        ...
        for bidx, (targets, GIs, clouds_data,edge_index) in enumerate(loader):
            ...
            embeddings = ptnCloudEmbedder.run(model, *clouds_data)
            adj = torch.sparse_coo_tensor(edge_index, torch.ones(edge_index.shape[1]), torch.Size([embeddings.shape[0], embeddings.shape[0]])).to_dense()  # [N,N]
            adj += torch.eye(adj.shape[0])
            adj = adj / adj.sum(dim=1, keepdim=True)  # [N,N]

            M = get_M(adj).cuda()
            adj = adj.clone().detach().cuda()
            with torch.no_grad():
                _, z = dae.gat(embeddings, adj, M)
            kmeans = KMeans(n_clusters=new_num_clusters, n_init=20)  # n_cluster 注意
            kmeans.fit_predict(z.data.cpu().numpy())
            dae.cluster_layer.data = torch.tensor(kmeans.cluster_centers_).cuda()
            A_pred, z, q = dae(embeddings, adj, M)
            outputs = model.ecc(z)
    ...
    def eval_final():
        ...
        for ss in range(args.test_multisamp_n):
            ...
            for bidx, (targets, GIs, clouds_data,edge_index) in enumerate(loader):
                ...
                embeddings = ptnCloudEmbedder.run(model, *clouds_data)
                adj = torch.sparse_coo_tensor(edge_index, torch.ones(edge_index.shape[1]), torch.Size(
                    [embeddings.shape[0], embeddings.shape[0]])).to_dense()  # [N,N]
                adj += torch.eye(adj.shape[0])
                adj = adj / adj.sum(dim=1, keepdim=True)  # [N,N]
                M = get_M(adj).cuda()
                adj = adj.clone().detach().cuda()
    ...
    if args.use_val_set :
        ...
        model, optimizer, stats = resume(args, dbinfo)
        torch.save({'epoch': epoch + 1, 'args': args, 'state_dict': model.state_dict(), 'optimizer' : optimizer.state_dict()},os.path.join(args.odir, 'model.pth.tar'))
...
def create_model(args, dbinfo):
    ...
    dae = DAEGC(num_features=32, hidden_size=256, embedding_size=32,alpha=0.2, num_clusters=7)
    dae.load_state_dict(torch.load(args.daemodel_path))
