...
#S3DIS specific
    ...
    #     xyz=np.ascontiguousarray(room_ver[:, 0:3], dtype='float32')
    # except:
    #     res=[i for i in range(len(room_ver))]
    #     for i in range(len(room_ver)):
    #         for j in range(len(room_ver[i])):
    #             # -9.000
    #             if '-9.1' in str(room_ver[i][j]):
    #                 room_ver[i][j]=float(str(room_ver[i][j])[:5])
    #
    #     room_ver=room_ver[res,:]
    #     xyz = np.ascontiguousarray(room_ver[:, 0:3], dtype='float32')
