...
parser.add_argument('--file_path', default='Area_5/WC_2', help='file to output (must include the area / set in its path)')
...
#fea_file = root + "features/" + folder + file_name

if not os.path.isfile(fea_file) or args.supervized_partition:
    fea_file   = root + "features_supervision/"          + folder + file_name + '.h5'
    #fea_file = root + "features_supervision/" + folder + file_name

spg_file   = root + "superpoint_graphs/" + folder + file_name + '.h5'
#spg_file = root + "superpoint_graphs/" + folder + file_name

ply_folder = root + "clouds/"            + folder
ply_folder_rgb = root + "clouds/"  + folder  +"rgb/"
ply_folder_gt = root + "clouds/"  + folder  +"gt/"
ply_folder_part = root + "clouds/"  + folder  +"part/"
ply_folder_pred = root + "clouds/"  + folder  +"pred/"
ply_file   = ply_folder                  + file_name
ply_file_rgb = ply_folder_rgb            + file_name
ply_file_gt  = ply_folder_gt             + file_name
ply_file_part= ply_folder_part           + file_name
ply_file_pred= ply_folder_pred           + file_name
...
if res_out or err_out:
    ...
    try:
        ...
        if (len(pred_red) != len(components)):
            print(len(pred_red),len(components))
...
if rgb_out:
    ...
    write_ply(ply_file_rgb + "_rgb.ply", xyz, rgb)

if gt_out:
    print("writing the GT file...")
    prediction2ply(ply_file_gt + "_GT.ply", xyz, labels, n_labels, args.dataset)
...
if par_out:
    ...
    partition2ply(ply_file_part + "_partition.ply", xyz, components)

if res_out and not bool(args.upsample):
    print("writing the prediction file...")
    prediction2ply(ply_file_pred + "_pred.ply", xyz, pred_full+1, n_labels,  args.dataset)
